...
export function StandardFilterGroup<T extends string>({
    ...
    return (
        <div className={`flex items-center gap-1 p-1 bg-white/50 dark:bg-neutral-900/50 backdrop-blur-md rounded-2xl border border-neutral-200 dark:border-neutral-800 shadow-sm h-12 overflow-x-auto scrollbar-hide no-scrollbar ${className}`}>
            {options.map((option) => (
                <button
                    key={option.id}
                    onClick={() => onSelect(option.id)}
                    className={`flex items-center justify-center gap-1.5 px-3 h-full rounded-xl text-xs font-bold transition-all duration-300 whitespace-nowrap active:scale-95 leading-none ${activeFilter === option.id ? currentTheme.active : currentTheme.inactive}`}
                >
