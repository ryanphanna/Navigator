...
export function StandardFilterGroup<T extends string>({
    ...
    return (
        <div className={`flex items-center gap-1.5 p-1.5 bg-white/50 dark:bg-neutral-900/50 backdrop-blur-md rounded-2.5xl border border-neutral-200 dark:border-neutral-800 shadow-sm overflow-x-auto scrollbar-hide no-scrollbar ${className}`}>
            {options.map((option) => (
                <button
                    key={option.id}
                    onClick={() => onSelect(option.id)}
                    className={`flex items-center gap-2 px-4 py-2.5 rounded-2xl text-xs font-bold transition-all duration-300 whitespace-nowrap active:scale-95 ${activeFilter === option.id ? currentTheme.active : currentTheme.inactive}`}
                >
