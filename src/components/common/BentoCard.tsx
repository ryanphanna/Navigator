...
export const BentoCard: React.FC<BentoCardProps> = ({
    ...
    return (
        <div
            ...
            className={`group relative bg-white dark:bg-neutral-900/40 backdrop-blur-2xl rounded-3xl p-5 border ${color.accent} shadow-sm hover:shadow-2xl hover:shadow-indigo-500/10 transition-all duration-500 text-left overflow-hidden h-full flex flex-col ${className}`}
        >
            ...
            <div className="flex items-center gap-4 relative z-10 mb-3">
                <div className={`w-12 h-12 ${color.iconBg} text-white rounded-2xl flex items-center justify-center shadow-lg shadow-indigo-500/10 group-hover:scale-110 group-hover:rotate-6 transition-all duration-500 ease-out`}>
                    <Icon className="w-6 h-6" />
                </div>
                <h3 className="text-xl font-black text-neutral-900 dark:text-white tracking-tight leading-none">
                    {title}
                </h3>
            </div>

            <p className="text-sm text-neutral-500 dark:text-neutral-400 leading-relaxed mb-4 relative z-10 font-medium">
                {description}
            </p>
            ...
            <div className="relative mt-auto pt-4 border-t border-neutral-100 dark:border-white/5">
                <div className="relative z-10 flex items-center justify-center min-h-[60px] transform group-hover:scale-105 transition-transform duration-700 ease-out">
                    {previewContent}
                </div>

                {/* Modern Action Bar */}
                <div
                    className={`mt-4 flex items-center justify-end gap-2 ${color.text} font-black text-[10px] tracking-[0.15em] uppercase transition-all relative z-10 cursor-pointer group/btn`}
                    onClick={onAction}
                >
