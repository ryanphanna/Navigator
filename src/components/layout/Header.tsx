...
import { TrendingUp, Briefcase, LogOut, Settings } from 'lucide-react';
            ...
            <div className="max-w-7xl mx-auto px-4 h-full flex items-center justify-between relative">
                ...
                {user && !isLoading && (
                    <nav className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 flex items-center bg-neutral-100/50 dark:bg-neutral-800/50 p-0.5 rounded-full border border-neutral-200/50 dark:border-neutral-700/50 transition-all duration-500 ease-in-out shadow-sm h-8">
                        {/* Navigation pill logic (simplified for snippet) */}
                        <button
                            onClick={() => onViewChange('job-fit')}
                            className={`relative z-10 px-3 h-full rounded-full text-xs font-bold flex items-center gap-1.5 ${currentView === 'job-fit' ? 'text-indigo-600' : 'text-neutral-500'}`}
                        >
